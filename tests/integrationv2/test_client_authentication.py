...
def assert_downgrade(results, protocol, is_complete=True):
    protocol_name = check_downgrade_openssl(protocol)
    if is_complete and protocol_name == Protocols.TLS12.name:
        assert to_bytes("Downgrade Successful. Actual protocol version: {}".format(
                protocol_name)) in results.stdout
    else:
        assert to_bytes("Actual protocol version: {}".format(
            protocol_name)) not in results.stdout
...
# @pytest.mark.parametrize("client_certificate", RSA_PSS_CERT, ids=get_parameter_name)
# def test_server_auth_downgrade(managed_process, provider, other_provider, protocol,
#                                cipher, certificate, client_certificate):
#     if "openssl1.0.2" in get_flag(S2N_PROVIDER_VERSION):
#         pytest.skip("s2n-tls skips")
#
#     port = next(available_ports)
#     random_bytes = data_bytes(64)
#     client_options = ProviderOptions(
...
#         assert b'read client certificate' not in results.stderr
#         assert b"peer did not return a certificate" in results.stderr
#         assert_s2n_handshake_complete(results, protocol, provider, True)
#
#     for results in client.get_results():
#         assert results.exception is None
#         if protocol is Protocols.TLS12:
#             assert b"Downgrade successful:" in results.stderr
#             assert_s2n_handshake_complete(results, protocol, provider, False)
...
def test_client_auth_with_s2n_server(managed_process, provider, other_provider, protocol, cipher, certificate,
    ...
    for results in server.get_results():
        ...
        protocol_name = check_downgrade_openssl(protocol)
        print("Check Downgrade", protocol_name)
